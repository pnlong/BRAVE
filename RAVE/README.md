...
## BRAVE vendored usage

In the [BRAVE](https://github.com/fcaspe/BRAVE) repo this tree lives at `BRAVE/RAVE/`. BRAVE does **not** install `acids-rave` from PyPI. From the BRAVE root, set `PYTHONPATH` and run scripts directly — see the [BRAVE README](../README.md) for install, preprocess, train, export, and W&B setup.
...
Training metrics are logged to **Weights & Biases** ([wandb guide](docs/wandb_guide.md)), not TensorBoard.
...
this will train a prior over the latent of the pretrained model `path/to/your/run`, and save the model and Weights & Biases logs to folder `/path/to/output`.
